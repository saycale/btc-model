...
> Примечание после аудита: интерактивная реализация от 9 сентября 2026 года
> переоценивает степенной тренд при обновлении `OBS`, отдельно датирует
> предварительную spot-точку и показывает HAC(12)-ошибку для регрессии по
> адресам. Численные выводы самого аудита ниже намеренно остаются привязаны к
> указанной ревизии; актуальные воспроизводимые числа лежат в `audit/results.json`.
